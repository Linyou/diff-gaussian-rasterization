...
__device__ void computeCov3D2(const glm::vec3 scale, float mod, const glm::vec4 rot, const glm::vec4 rot2, float* cov3D)
{
	// Create scaling matrix
	glm::mat3 S = glm::mat3(1.0f);
	S[0][0] = mod * scale.x;
...
// Perform initial steps for each Gaussian prior to rasterization.
template<int C>
__global__ void preprocessCUDA2(int P, int D, int M,
	const float* orig_points,
	const glm::vec3* scales,
	const float scale_modifier,
	const glm::vec4* rotations,
	...
	bool prefiltered)
{
	...
	{
		computeCov3D2(scales[idx], scale_modifier, rotations[idx], rotations2[idx], cov3Ds + idx * 6);
		cov3D = cov3Ds + idx * 6;
	}
	...
	float* __restrict__ out_depth)
{
	...
	float C[CHANNELS] = { 0 }, Depth = 0, Opacity = 0;
...
void FORWARD::preprocess2(int P, int D, int M,
	const float* means3D,
	const glm::vec3* scales,
	const float scale_modifier,
	const glm::vec4* rotations,
	...
	bool prefiltered)
{
	preprocessCUDA2<NUM_CHANNELS> << <(P + 255) / 256, 256 >> > (
		P, D, M,
		means3D,
		scales,
		scale_modifier,
